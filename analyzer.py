...
def has_dictionary_words(password, wordlist, min_word_len=4):
    ...
    for i in range(len(pw)):
        for j in range(i + min_word_len, len(pw) + 1):
            if pw[i:j] in wordlist:
                return True
    return False
...
COMMON_PASSWORDS = {
    ...
    'monkey', 'iloveyou', 'sunshine', 'master', 'welcome', 'admin',
    'password1', 'password123', 'root', 'test', 'shadow', 'football'
}
...
def detect_patterns(password):
    ...
    if pw_lower in COMMON_PASSWORDS or normalize_leet(password) in COMMON_PASSWORDS:
        found.append('common password')
...
PENALTIES = {
    'common password': 0.90,
    'dictionary words': 0.40,
    'keyboard walk': 0.30,
    'repeated characters': 0.30,
    'sequential characters': 0.20,
    'date pattern': 0.10,
}
...
def effective_entropy(password):
    ...
    total_penalty = min(sum(PENALTIES[p] for p in patterns), 0.95)
...
def analyze(password, verbose=False, no_hibp=False):
    ...
    if not no_hibp:
        if breaches is None:
            print("\n  ⚠️ hibp check failed (no internet?)")
        elif breaches == 0:
...
def main():
    ...
    if args.password:
        ...
        print("enter passwords to analyze (ctrl+c to quit)")
        try:
            while True:
                pw = input('\n> ').strip()
                if pw:
